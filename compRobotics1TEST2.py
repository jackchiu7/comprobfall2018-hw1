...
def main(filename, setNum = 0, fdaVal = False, vis = False): 
    filename = 'grid3.txt'
    global grid
    grid = makeGrid(filename, buff = .2)
    schruteFarms = getStartGoals(filename)
    ...
    numPlots = len(schruteFarms)
#    numPlots = 1
    
    fig, axes = plt.subplots(numPlots, 1,figsize = (10,numPlots*10))
    
    for vals in schruteFarms:
        print('start',vals[0],' : ', 'end',vals[1])    
    
    setPaths = []
#    fdaVal = True
    elapsedTime = []
    for idx in range(len(schruteFarms)):
        generateGridVertices()

        vals = schruteFarms[idx]
        ax = axes[idx]
#        ax = axes
        grid.plot(ax,interval)
        ...
        tempStr = 'start: ' + str(start[0]) + ',' + str(start[1]) + '  end: ' + str(goal[0]) + ',' +str(goal[1])
        ax.set_title(tempStr)
        ...
        if vis == False: 
            ...
            if pt1 != None:
                while pt1.parent != pt1:
                    pt2 = pt1.parent
                    tempPath.append(pt2)
                    circ = plt.Circle((pt2.x,pt2.y),grid.buff,color = 'yellow')
                    ax.add_artist(circ)
                    ax.plot([pt1.x,pt2.x], [pt1.y,pt2.y],'g-')
                    pt1 = pt2
            
                setPaths.append(tempPath)
            ...
            ptr = visResult
            visPath = [ptr]
            for i in range(0,len(visGraph)-1):
                for j in range(i+1,len(visGraph)):
                    if (line_of_sight_2(visGraph[i], visGraph[j])):
                        ax.plot([visGraph[i].x, visGraph[j].x], [visGraph[i].y, visGraph[j].y], 'r-', linewidth = .5, alpha = .5)
            ...
            while ptr.prev != ptr:
                ...
                #circ = plt.Circle((pt2.x,pt2.y),grid.buff,color = 'yellow')
                #ax.add_artist(circ)
                ax.plot([ptr.x,ptr2.x], [ptr.y,ptr2.y],'g-', linewidth = 2)
                ptr = ptr2
            
            setPaths.append(visPath)



    print 'fda = ', fdaVal, '   vis = ', vis, sum(elapsedTime)
        
    return grid, setPaths[setNum], sum(elapsedTime)
